...
async def event_store_session(engine: object) -> AsyncIterator[EventStore]:
    session_factory = async_sessionmaker(engine, expire_on_commit=False)  # type: ignore[arg-type]
    async with session_factory() as session:
        yield EventStore(session)


def build_test_app(event_store: object) -> TradingApp:
    return TradingApp(event_store=event_store, live_writes_enabled=False)  # type: ignore[arg-type]
